...
pub(crate) fn main() -> io::Result<()> {
    ...
    for node in nodes.iter() {
        ...
        for pair in pairs {
            // Add the antenna points to the unique locations
            unique_locations.insert(pair.0.clone(), true);
            unique_locations.insert(pair.1.clone(), true);

            let x_diff_one = pair.0.x - pair.1.x;
            let y_diff_one = pair.0.y - pair.1.y;

            let mut counter = 1;
            while pair.0.x + (counter * x_diff_one) >= 0
                && pair.0.x + (counter * x_diff_one) < max_x
                && pair.0.y + (counter * y_diff_one) >= 0
                && pair.0.y + (counter * y_diff_one) < max_y
            {
                unique_locations.insert(
                    Point {
                        x: pair.0.x + (counter * x_diff_one),
                        y: pair.0.y + (counter * y_diff_one),
                    },
                    true,
                );
                counter += 1;
            ...
            counter = 1;
            while pair.1.x + (counter * x_diff_two) >= 0
                && pair.1.x + (counter * x_diff_two) < max_x
                && pair.1.y + (counter * y_diff_two) >= 0
                && pair.1.y + (counter * y_diff_two) < max_y
            {
                unique_locations.insert(
                    Point {
                        x: pair.1.x + (counter * x_diff_two),
                        y: pair.1.y + (counter * y_diff_two),
                    },
                    true,
                );
                counter += 1;
